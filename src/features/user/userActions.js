...
export const setMainPhoto = photo => async (
  ...
  try {
    ...
    batch.update(userDocRef, {
      // batches the update, but does not yet execute
      photoURL: photo.url
    });
    ...
    let eventQuerySnap = await eventQuery.get(); // go get the events
...
export const getUserEvents = (userUid, activeTab) => async (
  ...
  switch (activeTab) {
    ...
    default:
      // all of the events for the given user
      query = eventsRef
        .where("userUid", "==", userUid)
        .orderBy("eventDate", "desc");
      break;
  ...
  try {
    ...
    for (let i = 0; i < querySnap.docs.length; i++) {
      let evt = await firestore
        ...
        .get();
      userEvents.push({ ...evt.data(), id: evt.id });
    }

    dispatch({ type: FETCH_USER_EVENTS, payload: { userEvents } });
    dispatch(asyncActionFinish());
  } catch (error) {
    console.log(error);
    dispatch(asyncActionError());
...
export const followUser = followee => async (
  dispatch,
  getState,
  { getFirestore }
) => {
    ...
    await firestore.set(
      {
        collection: "users",
        doc: user.uid,
        subcollections: [{ collection: "following", doc: followee.id }]
      },
      followRecord
    );

    dispatch(asyncActionFinish());
    toastr.success("Success", "You are now following this user");
  } catch (error) {
    console.log(error);
    dispatch(asyncActionError());
    toastr.error("Oops", "Problem following this user");
  }
};
